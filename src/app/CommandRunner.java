...
import statistics.UserStatisticsStrategy;
...
public final class CommandRunner {
    ...
    public static ObjectNode wrapped(final CommandInput commandInput) {
        WrappedResult wrapped = new UserStatisticsStrategy().generateStatistics(
                admin.getUser(commandInput.getUsername()));
    ...
    public static ObjectNode subscribe(final CommandInput commandInput) {
        ...
        String message;
        if (currentPage instanceof ArtistPage) {
            Artist artist = ((ArtistPage) currentPage).getArtist();
            message = artist.subscribe(user);
        } else {
    ...
    public static ObjectNode buyMerch(final CommandInput commandInput) {
        User user = admin.getUser(commandInput.getUsername());
        String message = user.buyMerch(commandInput);
        ObjectNode objectNode = objectMapper.createObjectNode();
        objectNode.put("command", commandInput.getCommand());
        objectNode.put("user", commandInput.getUsername());
        objectNode.put("timestamp", commandInput.getTimestamp());
    ...
    public static ObjectNode seeMerch(final CommandInput commandInput) {
        User user = admin.getUser(commandInput.getUsername());
        List boughtMerch = user.seeMerch(commandInput);

        ObjectNode objectNode = objectMapper.createObjectNode();
        objectNode.put("command", commandInput.getCommand());
        objectNode.put("user", commandInput.getUsername());
        objectNode.put("timestamp", commandInput.getTimestamp());
        objectNode.put("result", objectMapper.valueToTree(boughtMerch));
        return objectNode;
    }

    public static ObjectNode getNotifications(final CommandInput commandInput) {
        User user = admin.getUser(commandInput.getUsername());
        List<Notifications> notifications = user.getNotifications(); // Assuming Notification is a class representing your notifications
        ObjectNode objectNode = objectMapper.createObjectNode();
        objectNode.put("command", commandInput.getCommand());
