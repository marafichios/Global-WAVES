...
import statistics.WrappedResult;
...
public final class User extends UserAbstract implements Observer {
    ...
    @Getter
    @Setter
    private WrappedResult wrappedResult = new WrappedResult();
    @Getter
    @Setter
    private List<Notifications> notifications = new ArrayList<>();
    @Getter
    @Setter
    private Map<String, Subject> subscriptions = new HashMap<>();
    private List<Merchandise> purchasedMerch = new ArrayList<>();

    private List<Page> navigationHistory = new ArrayList<>();
    ...
    public void addPurchasedMerch(Merchandise merchandise) {
        purchasedMerch.add(merchandise);
    }

    public List<Merchandise> getPurchasedMerch() {
    ...
    public String buyMerch(final CommandInput commandInput) {
        ...
        // Check if user is on an artist's page
        ...
        artist.addPurchasedMerch(merch);
        user.addPurchasedMerch(merch);
    ...
    public List<String> seeMerch(final CommandInput commandInput) {
        ...
        User user = Admin.getInstance().getUser(username);
        if (user == null) {
            return List.of("The username " + username + " doesn't exist.");
        }

        List<String> purchasedItems = user.getPurchasedMerch().stream()
    ...
    public void navigateTo(Page page) {
        ...
        // Update the history index to point to the new page
    ...
    public String previousPage() {
        if (historyIndex > 0) {
            historyIndex--;
            setCurrentPage(navigationHistory.get(historyIndex));
